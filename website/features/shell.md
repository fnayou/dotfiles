...
| `speedtest.zsh` | `speed` / `speed-json` / `speed-log` / `speed-history` helpers (no-op without `cloudflare-speed-cli`) |
...
- `cloudflare-speed-cli` gets `speed` / `speed-json` / `speed-log` / `speed-history` wrappers — see
