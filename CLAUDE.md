...
- All `stow/common/` packages are stowed to `$HOME` (live symlinks into the repo), including `codex`. Home directory is modified by the existing symlinks. `stow/common/` remains the source of truth for which packages exist.
