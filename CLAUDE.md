...
- All pre-codex `stow/common/` packages are stowed to `$HOME` (live symlinks into the repo). The `codex` package has been added but is not yet stowed. Home directory is modified by the existing symlinks. `stow/common/` remains the source of truth for which packages exist.
