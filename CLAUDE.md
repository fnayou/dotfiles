...
- All `stow/common/` packages are stowed to `$HOME` (live symlinks into the repo), except `btop` (added, awaiting user stow). Home directory is modified by these symlinks. `stow/common/` remains the source of truth for which packages exist.
